...
def load_summary(db_mtime):
    with sqlite3.connect(DB_PATH) as con:
        ...
        SELECT a.symbol AS "نماد", a.company_name AS "نام شرکت",
               ...
               fl.floor96/1000000.0 AS "کف از ۱۳۹۶ (M$)",
               fl.floor96_date AS "تاریخ کف از ۱۳۹۶",
               CASE WHEN fl.floor96>0 THEN l.market_cap_usd_free/fl.floor96 END AS "نسبت فعلی به کف ۱۳۹۶",
...
with st.sidebar:
    ...
    pages=["تحلیل سهم","جدول نسبت به کف ۱۳۹۶","نمای کلی بازار","مقایسه سهم‌ها"]
...
if page=="تحلیل سهم":
    ...
    d96=d[d["Date_Shamsi"].astype(str).str.replace("-","/",regex=False).str[:4].apply(lambda x: x.isdigit() and int(x)>=1396)].copy()
    ...
    c4.metric("نسبت ارزش فعلی به کف از ۱۳۹۶",f'{current_to_trough:,.2f} برابر' if pd.notna(current_to_trough) else "—")
    ...
    a.metric("کف از ۱۳۹۶",f"{d96.loc[imin96,'MarketCap_USD_Free']/1e6:,.1f} M$" if imin96 is not None else "—")
    b.metric("تاریخ کف از ۱۳۹۶",d96.loc[imin96,"Date_Shamsi"] if imin96 is not None else "—")
    c.metric("سقف تاریخی",f"{free.max():,.1f} M$")
    e.metric("تاریخ سقف",d.loc[imax,"Date_Shamsi"])

elif page=="جدول نسبت به کف ۱۳۹۶":
    st.subheader("نسبت ارزش بازار دلاری فعلی به کمترین ارزش از سال ۱۳۹۶")
    st.caption("مبنای محاسبه دلار آزاد است. کف هر نماد فقط از ابتدای ۱۳۹۶ به بعد محاسبه می‌شود و جدول از کوچک‌ترین نسبت به بزرگ‌ترین مرتب شده است.")
    floor_table=summary[["نماد","نام شرکت","آخرین تاریخ","ارزش فعلی (M$)","کف از ۱۳۹۶ (M$)","تاریخ کف از ۱۳۹۶","نسبت فعلی به کف ۱۳۹۶"]].copy()
    floor_table=floor_table.dropna(subset=["نسبت فعلی به کف ۱۳۹۶"]).sort_values("نسبت فعلی به کف ۱۳۹۶",ascending=True)
    floor_table["ارزش فعلی (M$)"]=pd.to_numeric(floor_table["ارزش فعلی (M$)"],errors="coerce").round(2)
    floor_table["کف از ۱۳۹۶ (M$)"]=pd.to_numeric(floor_table["کف از ۱۳۹۶ (M$)"],errors="coerce").round(2)
    floor_table["نسبت فعلی به کف ۱۳۹۶"]=pd.to_numeric(floor_table["نسبت فعلی به کف ۱۳۹۶"],errors="coerce").round(3)
    st.dataframe(floor_table,use_container_width=True,hide_index=True,height=700,column_config={
        ...
        "کف از ۱۳۹۶ (M$)":st.column_config.NumberColumn(format="%.2f"),
        "نسبت فعلی به کف ۱۳۹۶":st.column_config.NumberColumn(format="%.3f ×"),
    })
    st.download_button("دانلود جدول (CSV)",floor_table.to_csv(index=False).encode("utf-8-sig"),"current_to_floor_since_1396.csv","text/csv")
    ...
    show=summary.copy()
    for c in ["ارزش فعلی (M$)","سقف تاریخی (M$)","کف از ۱۳۹۶ (M$)","نسبت فعلی به کف ۱۳۹۶","فاصله از سقف (%)"]: show[c]=show[c].round(1)
