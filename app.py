...
def _build_combined_db(base_path, addon_path, out_name, is_pe=False):
    """Build a temporary merged DB so GitHub can keep each source DB below 25 MiB."""
    if not base_path.exists():
        return base_path
    if not addon_path.exists():
        return base_path
    stamp=f"{base_path.stat().st_mtime_ns}_{addon_path.stat().st_mtime_ns}_{base_path.stat().st_size}_{addon_path.stat().st_size}"
    ...
    with sqlite3.connect(tmp) as con:
        ...
        if is_pe:
            con.execute("INSERT OR REPLACE INTO pe_history SELECT * FROM addon.pe_history")
        else:
            con.execute("INSERT OR REPLACE INTO stocks SELECT * FROM addon.stocks")
            con.execute("INSERT OR REPLACE INTO dates SELECT * FROM addon.dates")
            con.execute("INSERT OR REPLACE INTO history SELECT * FROM addon.history")
            con.execute("INSERT OR REPLACE INTO fx SELECT * FROM addon.fx")
        con.commit()
        con.execute("DETACH DATABASE addon")
    tmp.replace(out)
    marker.write_text(stamp)
    return out

DB_PATH=_build_combined_db(BASE_DB_PATH,MARKET_ADDON_PATH,"market_data_combined.db",False)
PE_DB_PATH=_build_combined_db(BASE_PE_DB_PATH,PE_ADDON_PATH,"pe_data_combined.db",True)
