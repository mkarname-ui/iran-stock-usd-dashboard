...
if page=="تحلیل سهم":
    ...
    trough=d.loc[d["MarketCap_USD_Free"] > 0, "MarketCap_USD_Free"].min()
    current_to_trough=(last["MarketCap_USD_Free"]/trough) if pd.notna(trough) and trough > 0 else float("nan")
    ...
    c4.metric("نسبت ارزش فعلی به کف تاریخی",f'{current_to_trough:,.2f} برابر' if pd.notna(current_to_trough) else "—")
