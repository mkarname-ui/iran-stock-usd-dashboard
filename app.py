...
BASE_DB_PATH=Path(__file__).with_name("market_data.db")
MARKET_ADDON_PATH=Path(__file__).with_name("market_data_4.db")
BASE_PE_DB_PATH=Path(__file__).with_name("pe_data.db")
PE_ADDON_PATH=Path(__file__).with_name("pe_data_4.db")
FUND_DB_PATH=Path(__file__).with_name("market_data_fundamental.db")

def _build_combined_market_db(base_path, addon_path, out_name):
    """Merge market add-on by natural keys, remapping stock/date IDs safely."""
    if not base_path.exists() or not addon_path.exists():
        return base_path
    stamp=f"v2_{base_path.stat().st_mtime_ns}_{addon_path.stat().st_mtime_ns}_{base_path.stat().st_size}_{addon_path.stat().st_size}"
    out=Path(tempfile.gettempdir())/out_name
    marker=Path(tempfile.gettempdir())/(out_name+".stamp")
    if out.exists() and marker.exists() and marker.read_text(errors="ignore")==stamp:
        return out
    tmp=Path(tempfile.gettempdir())/(out_name+".building")
    if tmp.exists(): tmp.unlink()
    shutil.copy2(base_path,tmp)
    with sqlite3.connect(tmp) as con:
        con.execute("ATTACH DATABASE ? AS addon",(str(addon_path),))
        # Insert natural dimensions first; never copy add-on integer IDs directly.
        con.execute("""INSERT OR IGNORE INTO stocks(symbol,company_name)
                       SELECT symbol,company_name FROM addon.stocks""")
        con.execute("""UPDATE stocks SET company_name=(SELECT a.company_name FROM addon.stocks a WHERE a.symbol=stocks.symbol)
                       WHERE symbol IN (SELECT symbol FROM addon.stocks)""")
        con.execute("""INSERT OR IGNORE INTO dates(gregorian_date,shamsi_date)
                       SELECT gregorian_date,shamsi_date FROM addon.dates""")
        # History: map add-on IDs -> symbol/date -> merged IDs.
        con.execute("""INSERT OR REPLACE INTO history(stock_id,date_id,market_cap_100b)
                       SELECT ms.stock_id, md.date_id, ah.market_cap_100b
                       FROM addon.history ah
                       JOIN addon.stocks ast ON ast.stock_id=ah.stock_id
                       JOIN addon.dates adt ON adt.date_id=ah.date_id
                       JOIN stocks ms ON ms.symbol=ast.symbol
                       JOIN dates md ON md.gregorian_date=adt.gregorian_date""")
        # FX: map by Gregorian date as well.
        con.execute("""INSERT OR REPLACE INTO fx(date_id,usd_free,usd_nima)
                       SELECT md.date_id, af.usd_free, af.usd_nima
                       FROM addon.fx af
                       JOIN addon.dates adt ON adt.date_id=af.date_id
                       JOIN dates md ON md.gregorian_date=adt.gregorian_date""")
        con.commit(); con.execute("DETACH DATABASE addon")
    tmp.replace(out); marker.write_text(stamp)
    return out

def _build_combined_pe_db(base_pe, addon_pe, market_addon, merged_market, out_name):
    """Merge P/E add-on while remapping its stock/date IDs through symbol and date."""
    if not base_pe.exists() or not addon_pe.exists() or not market_addon.exists():
        return base_pe
    stamp=f"v2_{base_pe.stat().st_mtime_ns}_{addon_pe.stat().st_mtime_ns}_{market_addon.stat().st_mtime_ns}"
    out=Path(tempfile.gettempdir())/out_name
    marker=Path(tempfile.gettempdir())/(out_name+".stamp")
    if out.exists() and marker.exists() and marker.read_text(errors="ignore")==stamp:
        return out
    tmp=Path(tempfile.gettempdir())/(out_name+".building")
    if tmp.exists(): tmp.unlink()
    shutil.copy2(base_pe,tmp)
    with sqlite3.connect(tmp) as con:
        con.execute("ATTACH DATABASE ? AS ape",(str(addon_pe),))
        con.execute("ATTACH DATABASE ? AS am",(str(market_addon),))
        con.execute("ATTACH DATABASE ? AS mm",(str(merged_market),))
        con.execute("""INSERT OR REPLACE INTO pe_history(stock_id,date_id,pe_e6)
                       SELECT ms.stock_id, md.date_id, ap.pe_e6
                       FROM ape.pe_history ap
                       JOIN am.stocks ast ON ast.stock_id=ap.stock_id
                       JOIN am.dates adt ON adt.date_id=ap.date_id
                       JOIN mm.stocks ms ON ms.symbol=ast.symbol
                       JOIN mm.dates md ON md.gregorian_date=adt.gregorian_date""")
        con.commit()
        con.execute("DETACH DATABASE ape"); con.execute("DETACH DATABASE am"); con.execute("DETACH DATABASE mm")
    tmp.replace(out); marker.write_text(stamp)
    return out

DB_PATH=_build_combined_market_db(BASE_DB_PATH,MARKET_ADDON_PATH,"market_data_combined_v2.db")
PE_DB_PATH=_build_combined_pe_db(BASE_PE_DB_PATH,PE_ADDON_PATH,MARKET_ADDON_PATH,DB_PATH,"pe_data_combined_v2.db")
...
def load_summary(db_mtime):
    with sqlite3.connect(DB_PATH) as con:
        ...
        period95_1404 AS (
          ...
          WHERE CAST(substr(replace(shamsi_date,'-','/'),1,4) AS INTEGER) >= 1395
            AND CAST(substr(replace(shamsi_date,'-','/'),1,4) AS INTEGER) <= 1404
            AND market_cap_usd_free > 0
        ),
        avgs AS (
          SELECT symbol, AVG(market_cap_usd_free) AS avg95_1404
          FROM period95_1404
          GROUP BY symbol
        ),
        agg AS (
        ...
        SELECT a.symbol AS "نماد", a.company_name AS "نام شرکت",
               ...
               av.avg95_1404/1000000.0 AS "میانگین ۱۳۹۵–۱۴۰۴ (M$)",
               CASE WHEN av.avg95_1404>0 THEN l.market_cap_usd_free/av.avg95_1404 END AS "نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴",
        ...
        LEFT JOIN avgs av ON av.symbol=a.symbol
...
def load_symbol(symbol, db_mtime):
    with sqlite3.connect(DB_PATH) as con:
        return pd.read_sql_query("""
        SELECT s.symbol AS Symbol, s.company_name AS Company, dt.date_id AS Date_ID,
               dt.gregorian_date AS Date_Gregorian, dt.shamsi_date AS Date_Shamsi,
               h.market_cap_100b*100000000000.0 AS MarketCap_Rial,
               f.usd_free AS USD_Free, f.usd_nima AS USD_NIMA,
               (h.market_cap_100b*100000000000.0)/f.usd_free AS MarketCap_USD_Free,
               (h.market_cap_100b*100000000000.0)/f.usd_nima AS MarketCap_USD_NIMA
        FROM history h JOIN stocks s ON s.stock_id=h.stock_id
        JOIN dates dt ON dt.date_id=h.date_id JOIN fx f ON f.date_id=h.date_id
        WHERE s.symbol=? ORDER BY dt.gregorian_date
        """,con,params=(symbol,))

@st.cache_data(show_spinner=False)
def load_pe(symbol, db_mtime, pe_mtime):
    if not PE_DB_PATH.exists(): return pd.DataFrame(columns=["Date_ID","PE_TTM"])
    with sqlite3.connect(DB_PATH) as con:
        row=con.execute("SELECT stock_id FROM stocks WHERE symbol=?",(symbol,)).fetchone()
    if row is None: return pd.DataFrame(columns=["Date_ID","PE_TTM"])
    with sqlite3.connect(PE_DB_PATH) as con:
        return pd.read_sql_query("SELECT date_id AS Date_ID, pe_e6/1000000.0 AS PE_TTM FROM pe_history WHERE stock_id=? ORDER BY date_id",con,params=(row[0],))

@st.cache_data(show_spinner=False)
def load_latest_pe5_table(db_mtime, pe_db_mtime):
    """Latest 5-year USD P/E for every symbol, using the same free-USD methodology as the chart."""
    if not PE_DB_PATH.exists():
        return pd.DataFrame(columns=["نماد","P/E دلاری ۵ ساله (آزاد)"])
    with sqlite3.connect(DB_PATH) as con:
        con.execute("ATTACH DATABASE ? AS pedb", (str(PE_DB_PATH),))
        q = """
        WITH last_dates AS (
          SELECT h.stock_id, MAX(dt.gregorian_date) AS last_date
          FROM history h JOIN dates dt ON dt.date_id=h.date_id
          GROUP BY h.stock_id
        ),
        pe_span AS (
          SELECT p.stock_id, MIN(dt.gregorian_date) AS first_pe_date
          FROM pedb.pe_history p JOIN dates dt ON dt.date_id=p.date_id
          WHERE p.pe_e6 IS NOT NULL AND p.pe_e6<>0
          GROUP BY p.stock_id
        ),
        five_year_coverage AS (
          /* A symbol is eligible only when every market observation in its 5-year
             window that has a usable FX rate also has an actual, non-zero P/E.
             This prevents loss-making periods whose source P/E is blank from
             silently disappearing from the average-profit calculation. */
          SELECT h.stock_id,
                 COUNT(*) AS market_rows,
                 SUM(CASE WHEN p.pe_e6 IS NOT NULL AND p.pe_e6<>0 THEN 1 ELSE 0 END) AS pe_rows
          FROM history h
          JOIN dates dt ON dt.date_id=h.date_id
          JOIN fx f ON f.date_id=h.date_id AND f.usd_free>0
          JOIN last_dates l ON l.stock_id=h.stock_id
          LEFT JOIN pedb.pe_history p ON p.stock_id=h.stock_id AND p.date_id=h.date_id
          WHERE date(dt.gregorian_date) >= date(l.last_date,'-5 years')
            AND date(dt.gregorian_date) <= date(l.last_date)
          GROUP BY h.stock_id
        ),
        avg_profit AS (
          SELECT h.stock_id,
                 AVG(((h.market_cap_100b*100000000000.0)/(p.pe_e6/1000000.0))/f.usd_free) AS avg5_profit_usd_free
          FROM history h
          JOIN dates dt ON dt.date_id=h.date_id
          JOIN fx f ON f.date_id=h.date_id
          JOIN pedb.pe_history p ON p.stock_id=h.stock_id AND p.date_id=h.date_id
          JOIN last_dates l ON l.stock_id=h.stock_id
          JOIN five_year_coverage cv ON cv.stock_id=h.stock_id AND cv.market_rows=cv.pe_rows AND cv.market_rows>0
          WHERE p.pe_e6 IS NOT NULL AND p.pe_e6<>0
            AND f.usd_free>0
            AND date(dt.gregorian_date) >= date(l.last_date,'-5 years')
            AND date(dt.gregorian_date) <= date(l.last_date)
          GROUP BY h.stock_id
        ),
        current_mc AS (
          SELECT h.stock_id, (h.market_cap_100b*100000000000.0)/f.usd_free AS current_mc_usd_free
          FROM history h
          JOIN dates dt ON dt.date_id=h.date_id
          JOIN fx f ON f.date_id=h.date_id
          JOIN last_dates l ON l.stock_id=h.stock_id AND l.last_date=dt.gregorian_date
        )
        SELECT s.symbol AS "نماد",
               CASE WHEN a.avg5_profit_usd_free<>0
                         AND date(ps.first_pe_date) <= date(l.last_date,'-5 years')
                    THEN c.current_mc_usd_free/a.avg5_profit_usd_free END AS "P/E دلاری ۵ ساله (آزاد)"
        FROM stocks s
        JOIN last_dates l ON l.stock_id=s.stock_id
        LEFT JOIN pe_span ps ON ps.stock_id=s.stock_id
        LEFT JOIN avg_profit a ON a.stock_id=s.stock_id
        LEFT JOIN current_mc c ON c.stock_id=s.stock_id
        """
        out=pd.read_sql_query(q,con)
        con.execute("DETACH DATABASE pedb")
    return out

def pe_mtime():
    return PE_DB_PATH.stat().st_mtime_ns if PE_DB_PATH.exists() else 0
...
with st.sidebar:
    ...
    pages=["تحلیل سهم","جدول نسبت به میانگین ۱۳۹۵–۱۴۰۴","نمای کلی بازار","مقایسه سهم‌ها"]
...
if page=="تحلیل سهم":
    ...
    d95_1404=d[d["Date_Shamsi"].astype(str).str.replace("-","/",regex=False).str[:4].apply(lambda x: x.isdigit() and 1395<=int(x)<=1404)].copy()
    d95_1404=d95_1404[d95_1404["MarketCap_USD_Free"]>0]
    avg95_1404=d95_1404["MarketCap_USD_Free"].mean() if not d95_1404.empty else float("nan")
    current_to_avg=(last["MarketCap_USD_Free"]/avg95_1404) if pd.notna(avg95_1404) and avg95_1404 > 0 else float("nan")
    ...
    c4.metric("نسبت ارزش فعلی به میانگین ۱۳۹۵–۱۴۰۴",f'{current_to_avg:,.2f} برابر' if pd.notna(current_to_avg) else "—")
    ...
    fig=px.line(long,x="Date_Shamsi",y="ارزش بازار (میلیون دلار)",color="مبنای ارز",hover_data={"Date_Gregorian":False},title=f'{sym} — {last["Company"]}', category_orders={"Date_Shamsi": p.sort_values("Date_Gregorian")["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
    fig.update_xaxes(title="تاریخ شمسی", type="category", nticks=12)
    ...
    # نمودار دوم: همان ارزش بازار دلاری، با حذف داده‌های زمستان ۱۳۹۸ تا پایان پاییز ۱۳۹۹.
    # بازه حذف‌شده: ۱۳۹۸/۱۰/۰۱ تا ۱۳۹۹/۰۹/۳۰ (شامل ابتدا و انتها).
    def shamsi_key(v):
    ...
    p_gap=p_gap[~p_gap["_sh_key"].between(13981001,13990930,inclusive="both")].drop(columns=["_sh_key"])
    ...
    fig_gap=px.line(long_gap,x="Date_Shamsi",y="ارزش بازار (میلیون دلار)",color="مبنای ارز",hover_data={"Date_Gregorian":False},title=f'{sym} — ارزش بازار دلاری با حذف زمستان ۱۳۹۸ تا پایان پاییز ۱۳۹۹', category_orders={"Date_Shamsi": p_gap.sort_values("Date_Gregorian")["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
    fig_gap.update_xaxes(title="تاریخ شمسی", type="category", nticks=12)
    ...
    st.markdown("### P/E و سود TTM")
    if PE_DB_PATH.exists():
        pe=load_pe(sym,db_mtime(),pe_mtime())
        ep=d.merge(pe,on="Date_ID",how="inner")
        ep=ep[pd.to_numeric(ep["PE_TTM"],errors="coerce").notna() & (ep["PE_TTM"]!=0)].copy()
        ep["PE_TTM"]=pd.to_numeric(ep["PE_TTM"],errors="coerce")
        ep["Profit_Rial_TTM"]=ep["MarketCap_Rial"]/ep["PE_TTM"]
        ep["Profit_USD_Free_TTM"]=ep["Profit_Rial_TTM"]/ep["USD_Free"]
        ep["Profit_USD_NIMA_TTM"]=ep["Profit_Rial_TTM"]/ep["USD_NIMA"]
        ep["Date_Gregorian"]=pd.to_datetime(ep["Date_Gregorian"],errors="coerce")
        ep=ep.sort_values("Date_Gregorian")
        if not ep.empty:
            fig_pe=px.line(ep,x="Date_Shamsi",y="PE_TTM",title=f"{sym} — نسبت P/E-TTM",category_orders={"Date_Shamsi":ep["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
            fig_pe.update_xaxes(title="تاریخ شمسی",type="category",nticks=14)
            fig_pe.update_layout(height=500,hovermode="x unified",yaxis_title="P/E-TTM",showlegend=False)
            st.plotly_chart(fig_pe,use_container_width=True,config={"displayModeBar":False,"responsive":True})
            profit=ep[["Date_Gregorian","Date_Shamsi","Profit_USD_Free_TTM","Profit_USD_NIMA_TTM"]].copy()
            profit["دلار آزاد"]=profit["Profit_USD_Free_TTM"]/1e6; profit["دلار نیمایی"]=profit["Profit_USD_NIMA_TTM"]/1e6
            profit_long=profit.melt(id_vars=["Date_Gregorian","Date_Shamsi"],value_vars=["دلار آزاد","دلار نیمایی"],var_name="مبنای ارز",value_name="سود TTM (میلیون دلار)")
            fig_profit=px.line(profit_long,x="Date_Shamsi",y="سود TTM (میلیون دلار)",color="مبنای ارز",title=f"{sym} — سود TTM دلاری محاسبه‌شده از P/E",category_orders={"Date_Shamsi":ep["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
            fig_profit.update_xaxes(title="تاریخ شمسی",type="category",nticks=14)
            fig_profit.update_layout(height=520,hovermode="x unified",legend_title_text="",yaxis_title="میلیون دلار")
            st.plotly_chart(fig_profit,use_container_width=True,config={"displayModeBar":False,"responsive":True})

            # P/E دلاری بر مبنای میانگین متحرک ۵ ساله سود TTM دلاری
            # میانگین فقط وقتی نمایش داده می‌شود که برای آن نماد حداقل ۵ سال سابقه تا آن تاریخ وجود داشته باشد.
            ep5=ep.dropna(subset=["Date_Gregorian"]).copy().sort_values("Date_Gregorian")
            ep5=ep5.set_index("Date_Gregorian")
            ep5["Avg5Y_Profit_USD_Free"]=ep5["Profit_USD_Free_TTM"].rolling("1826D",min_periods=1).mean()
            ep5["Avg5Y_Profit_USD_NIMA"]=ep5["Profit_USD_NIMA_TTM"].rolling("1826D",min_periods=1).mean()
            first_date=ep5.index.min()
            full5_mask=ep5.index >= (first_date + pd.DateOffset(years=5))
            ep5.loc[~full5_mask,["Avg5Y_Profit_USD_Free","Avg5Y_Profit_USD_NIMA"]]=float("nan")
            ep5["PE_USD_5Y_Free"]=ep5["MarketCap_USD_Free"]/ep5["Avg5Y_Profit_USD_Free"]
            ep5["PE_USD_5Y_NIMA"]=ep5["MarketCap_USD_NIMA"]/ep5["Avg5Y_Profit_USD_NIMA"]
            ep5=ep5.replace([float("inf"),-float("inf")],float("nan")).reset_index()
            pe5=ep5[["Date_Gregorian","Date_Shamsi","PE_USD_5Y_Free","PE_USD_5Y_NIMA"]].dropna(how="all",subset=["PE_USD_5Y_Free","PE_USD_5Y_NIMA"]).copy()
            if not pe5.empty:
                pe5["دلار آزاد"]=pe5["PE_USD_5Y_Free"]
                pe5["دلار نیمایی"]=pe5["PE_USD_5Y_NIMA"]
                pe5_long=pe5.melt(id_vars=["Date_Gregorian","Date_Shamsi"],value_vars=["دلار آزاد","دلار نیمایی"],var_name="مبنای ارز",value_name="P/E دلاری ۵ ساله")
                fig_pe5=px.line(pe5_long,x="Date_Shamsi",y="P/E دلاری ۵ ساله",color="مبنای ارز",title=f"{sym} — P/E دلاری بر مبنای میانگین سود ۵ ساله",category_orders={"Date_Shamsi":pe5.sort_values("Date_Gregorian")["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
                fig_pe5.update_xaxes(title="تاریخ شمسی",type="category",nticks=14)
                fig_pe5.update_layout(height=520,hovermode="x unified",legend_title_text="",yaxis_title="P/E دلاری ۵ ساله")
                st.plotly_chart(fig_pe5,use_container_width=True,config={"displayModeBar":False,"responsive":True})
                st.caption("P/E دلاری ۵ ساله = ارزش بازار دلاری همان روز ÷ میانگین سود TTM دلاری در ۵ سال گذشته. این نسبت جداگانه با دلار آزاد و دلار نیمایی محاسبه می‌شود و فقط پس از تکمیل حداقل ۵ سال سابقه برای نماد نمایش داده می‌شود.")
            else:
                st.info("برای محاسبه P/E دلاری ۵ ساله این نماد هنوز سابقه کافی وجود ندارد.")

            st.caption("سود TTM ریالی شرکت = ارزش بازار ریالی ÷ P/E-TTM. سپس با نرخ دلار آزاد و نیمایی همان تاریخ به دلار تبدیل شده است. P/E منفی حذف نشده و زیان TTM را به‌صورت منفی نشان می‌دهد؛ P/E صفر یا خالی قابل محاسبه نیست.")
        else: st.info("برای این نماد داده P/E-TTM قابل محاسبه موجود نیست.")
    else: st.warning("فایل pe_data.db کنار app.py پیدا نشد.")

    free=d["MarketCap_USD_Free"]/1e6
    imax=d["MarketCap_USD_Free"].idxmax()
    a,b,c=st.columns(3)
    a.metric("میانگین ارزش بازار ۱۳۹۵–۱۴۰۴",f"{avg95_1404/1e6:,.1f} M$" if pd.notna(avg95_1404) else "—")
    b.metric("سقف تاریخی",f"{free.max():,.1f} M$")
    c.metric("تاریخ سقف",d.loc[imax,"Date_Shamsi"])

elif page=="جدول نسبت به میانگین ۱۳۹۵–۱۴۰۴":
    st.subheader("نسبت ارزش بازار دلاری فعلی به میانگین ارزش بازار دلاری ۱۳۹۵ تا پایان ۱۴۰۴")
    st.caption("مبنای جدول دلار آزاد است. فقط نمادهایی نمایش داده می‌شوند که داده P/E لازم در کل پنجره ۵ ساله‌شان موجود باشد؛ دوره‌های فاقد P/E از محاسبه میانگین حذف و پنهان نمی‌شوند، بلکه باعث حذف نماد از این جدول می‌شوند. جدول بر اساس نسبت اول از کوچک به بزرگ مرتب است.")
    avg_table=summary[["نماد","نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴","ارزش فعلی (M$)","میانگین ۱۳۹۵–۱۴۰۴ (M$)","آخرین تاریخ"]].copy()
    pe5_now=load_latest_pe5_table(db_mtime(),pe_mtime())
    avg_table=avg_table.merge(pe5_now,on="نماد",how="left")
    # فقط نمادهایی وارد جدول می‌شوند که در کل پنجره ۵ ساله، برای تمام روزهای دارای ارزش بازار و نرخ ارز، P/E واقعی و غیرصفر داشته باشند.
    avg_table["P/E دلاری ۵ ساله (آزاد)"]=pd.to_numeric(avg_table["P/E دلاری ۵ ساله (آزاد)"],errors="coerce")
    avg_table=avg_table.dropna(subset=["P/E دلاری ۵ ساله (آزاد)"])
    avg_table["حاصل‌ضرب دو نسبت"]=avg_table["نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴"]*avg_table["P/E دلاری ۵ ساله (آزاد)"]
    # برای خوانایی موبایل، سه ستون تحلیلی بلافاصله بعد از نماد قرار می‌گیرند.
    avg_table=avg_table[["نماد","نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴","P/E دلاری ۵ ساله (آزاد)","حاصل‌ضرب دو نسبت","ارزش فعلی (M$)","میانگین ۱۳۹۵–۱۴۰۴ (M$)","آخرین تاریخ"]]
    avg_table=avg_table.dropna(subset=["نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴"]).sort_values("نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴",ascending=True)
    avg_table["ارزش فعلی (M$)"]=pd.to_numeric(avg_table["ارزش فعلی (M$)"],errors="coerce").round(2)
    avg_table["میانگین ۱۳۹۵–۱۴۰۴ (M$)"]=pd.to_numeric(avg_table["میانگین ۱۳۹۵–۱۴۰۴ (M$)"],errors="coerce").round(2)
    avg_table["نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴"]=pd.to_numeric(avg_table["نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴"],errors="coerce").round(3)
    avg_table["P/E دلاری ۵ ساله (آزاد)"]=pd.to_numeric(avg_table["P/E دلاری ۵ ساله (آزاد)"],errors="coerce").round(3)
    avg_table["حاصل‌ضرب دو نسبت"]=pd.to_numeric(avg_table["حاصل‌ضرب دو نسبت"],errors="coerce").round(3)
    st.dataframe(avg_table,use_container_width=True,hide_index=True,height=700,column_config={
        "ارزش فعلی (M$)":st.column_config.NumberColumn(format="%.2f"),
        "میانگین ۱۳۹۵–۱۴۰۴ (M$)":st.column_config.NumberColumn(format="%.2f"),
        "نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴":st.column_config.NumberColumn(format="%.3f ×"),
        "P/E دلاری ۵ ساله (آزاد)":st.column_config.NumberColumn(format="%.3f ×"),
        "حاصل‌ضرب دو نسبت":st.column_config.NumberColumn(format="%.3f"),
    })
    st.download_button("دانلود جدول (CSV)",avg_table.to_csv(index=False).encode("utf-8-sig"),"current_to_average_1395_1404.csv","text/csv")
    ...
    show=summary.copy()
    for c in ["ارزش فعلی (M$)","سقف تاریخی (M$)","میانگین ۱۳۹۵–۱۴۰۴ (M$)","نسبت فعلی به میانگین ۱۳۹۵–۱۴۰۴","فاصله از سقف (%)"]: show[c]=show[c].round(1)
    ...
    if picked:
        ...
        fig=px.line(cmp,x="Date_Shamsi",y="ارزش بازار (میلیون دلار)",color="Symbol",hover_data={"Date_Gregorian":False},title="مقایسه ارزش بازار دلاری — دلار آزاد", category_orders={"Date_Shamsi": cmp.sort_values("Date_Gregorian")["Date_Shamsi"].astype(str).drop_duplicates().tolist()})
        fig.update_xaxes(title="تاریخ شمسی", type="category", nticks=12)
