...
[data-testid="stPlotlyChart"]{direction:ltr!important;overflow:hidden!important;max-width:100%!important}
[data-testid="stPlotlyChart"]>div{max-width:100%!important;overflow:hidden!important}
[data-testid="stHeader"],[data-testid="stToolbar"],[data-testid="stDecoration"],[data-testid="stStatusWidget"]{direction:ltr!important}
@media (max-width:768px){.block-container{padding-left:.75rem!important;padding-right:.75rem!important}h1{font-size:1.7rem!important;line-height:1.5!important}}
</style>
...
if page=="تحلیل سهم":
    ...
    st.caption("همه نمادهای بازار در جدول حضور دارند. نسبت‌های بنیادی فقط در تاریخ‌های متناظر با پایان دوره مالی نمایش داده می‌شوند؛ برای نمادهایی که هنوز داده بنیادی وارد نشده، این دو ستون خالی است.")
    ...
    all_symbols=sorted(ts["نماد"].dropna().unique().tolist())
    ...
    only_fund=st.checkbox(
        "فقط تاریخ‌های دارای نسبت بنیادی",
        value=False,
        help="اگر فعال شود، فعلاً فقط شرکت‌هایی که داده بنیادی آنها وارد شده نمایش داده می‌شوند."
    )

    view=ts[ts["نماد"].isin(selected)].copy()
    if only_fund:
        view=view[
            view["موجودی کالا / ارزش بازار (%)"].notna() |
            view["(دارایی جاری - کل بدهی‌ها) / ارزش بازار (%)"].notna()
        ].copy()
