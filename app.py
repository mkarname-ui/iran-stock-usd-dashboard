...
if page=="تحلیل سهم":
    ...
    fig=px.line(long,x="Date_Gregorian",y="ارزش بازار (میلیون دلار)",color="مبنای ارز",custom_data=["Date_Shamsi"],title=f'{sym} — {last["Company"]}')
    fig.update_traces(hovertemplate="<b>%{customdata[0]}</b><br>%{y:,.1f} میلیون دلار<extra></extra>")
    fig.update_layout(height=500,hovermode="x unified",legend_title_text="",xaxis_title="تاریخ",yaxis_title="میلیون دلار",margin=dict(l=10,r=10,t=55,b=20),legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1))
    ...
    # محور افقی نمودارها میلادی/زمانی است تا در موبایل خطوط پرشی ایجاد نشود؛
    # تاریخ مالی شمسی در hover نمایش داده می‌شود.
    ratios["MarketDate_Gregorian"]=pd.to_datetime(ratios["MarketDate"].map(lambda x: None),errors="coerce")
    with db_connect_readonly() as con:
        md=pd.read_sql_query("""
            SELECT period_end_month AS Period, market_date_gregorian AS MarketDate_Gregorian
            FROM v_key_fundamental_ratios WHERE symbol=? ORDER BY period_end_month
        """,con,params=(sym,))
    md["MarketDate_Gregorian"]=pd.to_datetime(md["MarketDate_Gregorian"],errors="coerce")
    ratios=ratios.drop(columns=["MarketDate_Gregorian"]).merge(md,on="Period",how="left")
    ratios=ratios.dropna(subset=["MarketDate_Gregorian"]).sort_values("MarketDate_Gregorian")
    ...
    selected=st.multiselect("متغیرهای ترازنامه‌ای",list(metric_map),default=["موجودی / ارزش بازار"])
    if selected:
        cols=["MarketDate_Gregorian","Period"]+[metric_map[x] for x in selected]
        pp=ratios[cols].rename(columns={v:k for k,v in metric_map.items()})
        long=pp.melt(id_vars=["MarketDate_Gregorian","Period"],var_name="متغیر",value_name="نسبت").dropna(subset=["نسبت"])
        fig=px.line(long,x="MarketDate_Gregorian",y="نسبت",color="متغیر",markers=True,
                    custom_data=["Period"],title=f"{sym} — نسبت‌های بنیادی به ارزش بازار")
        fig.update_traces(hovertemplate="<b>دوره %{customdata[0]}</b><br>%{y:.1%}<extra>%{fullData.name}</extra>")
        fig.update_layout(height=500,hovermode="x unified",legend_title_text="",yaxis_tickformat=".0%",
                          xaxis_title="تاریخ",yaxis_title="نسبت",margin=dict(l=10,r=10,t=55,b=20),
    ...
    st.markdown("### موجودی و ارزش بازار در مقاطع مالی")
    amounts=ratios[["MarketDate_Gregorian","Period","inventory","MarketCap_MRial"]].copy()
    amounts=amounts.rename(columns={"inventory":"موجودی (میلیون ریال)","MarketCap_MRial":"ارزش بازار (میلیون ریال)"})
    along=amounts.melt(id_vars=["MarketDate_Gregorian","Period"],
                       value_vars=["موجودی (میلیون ریال)","ارزش بازار (میلیون ریال)"],
                       var_name="متغیر",value_name="مبلغ").dropna(subset=["مبلغ"])
    fig2=px.line(along,x="MarketDate_Gregorian",y="مبلغ",color="متغیر",markers=True,custom_data=["Period"])
    fig2.update_traces(hovertemplate="<b>دوره %{customdata[0]}</b><br>%{y:,.0f} میلیون ریال<extra>%{fullData.name}</extra>")
    fig2.update_layout(height=450,hovermode="x unified",legend_title_text="",xaxis_title="تاریخ",
                       yaxis_title="میلیون ریال",margin=dict(l=10,r=10,t=25,b=20),
                       legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1))
    fig2.update_xaxes(nticks=8,tickformat="%Y")
    st.plotly_chart(fig2,use_container_width=True,config={"displayModeBar":False})

    table=ratios[["Period","MarketDate","inventory","MarketCap_MRial","inventory_to_mcap"]].copy()
    table.columns=["دوره مالی","روز معاملاتی مبنا","موجودی (میلیون ریال)","ارزش بازار (میلیون ریال)","موجودی / ارزش بازار"]
    table["موجودی / ارزش بازار"]=(table["موجودی / ارزش بازار"]*100).round(1)
    st.dataframe(table.sort_values("دوره مالی",ascending=False),use_container_width=True,hide_index=True)
    st.caption("برای هر دوره مالی، ارزش بازار آخرین روز معاملاتی موجود تا پایان همان دوره استفاده شده است. مقادیر موجودی از داده‌های ترازنامه‌ای ثبت‌شده در پایگاه داده بنیادی خوانده می‌شوند.")
    ...
    if picked:
        ...
        fig=px.line(cmp,x="Date_Gregorian",y="ارزش بازار (میلیون دلار)",color="Symbol",custom_data=["Date_Shamsi"],title="مقایسه ارزش بازار دلاری — دلار آزاد")
        fig.update_traces(hovertemplate="<b>%{customdata[0]}</b><br>%{y:,.1f} میلیون دلار<extra>%{fullData.name}</extra>")
        fig.update_layout(height=520,hovermode="x unified",legend_title_text="نماد",xaxis_title="تاریخ",yaxis_title="میلیون دلار",margin=dict(l=10,r=10,t=55,b=20))
        fig.update_xaxes(nticks=8,tickformat="%Y")
        st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
